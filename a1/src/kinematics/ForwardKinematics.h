...
Matrix2d dendEffector_dangles(const Linkage &linkage, const Vector2d &angles) {
    ...
    dp2_dangles << -linkage.length[0]*sin(angles[0])+linkage.length[1]*(-sin(angles[0])*cos(angles[1])-cos(angles[0])*sin(angles[1])), linkage.length[1]*(-cos(angles[0])*sin(angles[1])-sin(angles[0])*cos(angles[1])),
                    linkage.length[0]*cos(angles[0])+linkage.length[1]*(cos(angles[0])*cos(angles[1])-sin(angles[0])*sin(angles[1])), linkage.length[1]*(-sin(angles[0])*sin(angles[1])+cos(angles[0])*cos(angles[1]));
...
Tensor2x2x2 ddendEffector_ddangles(const Linkage &linkage, const Vector2d &angles) {
    ...
    tensor[0] << -linkage.length[0]*cos(angles[0])-linkage.length[1]*cos(angles[0]+angles[1]), -linkage.length[1]*cos(angles[0]+angles[1]),
                 -linkage.length[0]*sin(angles[0])-linkage.length[1]*sin(angles[0]+angles[1]),-linkage.length[1]*sin(angles[0]+angles[1]);
    tensor[1] << -linkage.length[1]*cos(angles[0]+angles[1]), -linkage.length[1]*cos(angles[0]+angles[1]),
                 -linkage.length[1]*sin(angles[0]+angles[1]),-linkage.length[1]*sin(angles[0]+angles[1]);
    return tensor;
}
