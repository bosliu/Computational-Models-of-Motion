...
public:
	...
	virtual double evaluate(const VectorXd &stack) const {
		...
		for (int k =0; k < K; k++)
		{
			O += pow(10., 3.0) * (h * v[k] - (x[k+1] - x[k])).squaredNorm();
			O += pow(10., 3.0) * (h * get_Fk(u[k], x[k]) / m - (v[k + 1] - v[k])).squaredNorm();
		}
		
		O += pow(10., 3.0) * (x[0] - x0). squaredNorm();
...
private:
	...
	void drawImGui() override {
		...
		Separator();
		if(Checkbox("I_AM_HAVING_SCREEN_ISSUES", &I_AM_HAVING_SCREEN_ISSUES)){
			if(I_AM_HAVING_SCREEN_ISSUES){
				screen_offset = Vector2d(0.5, 1.5);
				ZOOM_ = 0.5;
			}				
			else{
				screen_offset = Vector2d(1, 1);
				ZOOM_ = 1.0;
			}				
		}
		InputScalarN("Screen offset", ImGuiDataType_Double, &screen_offset, 2);
		static double OFFSET_MIN_ = 0;
		static double OFFSET_MAX_ = 2;
		SliderScalar("Screen x offset", ImGuiDataType_Double, &screen_offset[0], &OFFSET_MIN_, &OFFSET_MAX_);
		SliderScalar("Screen y offset", ImGuiDataType_Double, &screen_offset[1], &OFFSET_MIN_, &OFFSET_MAX_);
		SliderScalar("ZOOM", ImGuiDataType_Double, &ZOOM_, &ZOOM_MIN_, &ZOOM_MAX_); 
		InputDouble("ZOOM###zoom_box", &ZOOM_);
		Separator();
...
protected:
	...
	void mouseMove(double, double) override {
		double f = 1.; x_mouse = _2xy(Vector2d(f*mouseState.lastMouseX, f*mouseState.lastMouseY));
		// --
...
int main(int, char**) {
	TranscriptionApp app(720, 720, "TranscriptionApp",  2.f);
	app.run();
	return 0;
}
