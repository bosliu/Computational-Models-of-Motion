    ...
    Trajectory3D generateLimbTrajectory(RobotLimb* limb,
        ...
        while (t < tEnd) {
            ...
            if (cpi.isStance()) {
                ...
                double offset = std::max(sqrt(pow(11.5, 2) - (pow(eePos.x(), 2) + pow(eePos.z() - 6, 2))) - 11, 0.0);
...
private:
    ...
    void generate(const bFrameState& startingbFrameState) {
        ...
        while (t < tEnd) {
            ...
            double offset = std::max(sqrt(pow(11.5, 2) - (pow(pos.x, 2) + pow(pos.z - 6, 2))) - 11, 0.0);
            ...
            V3D pCurrent = V3D(vSideways * dt, 0, vForward * dt);
            V3D pWorld =  heading * pCurrent;
            pos.z = pos.z + pWorld[2];
            pos.x = pos.x + pWorld[0];
            
             /* + TODO: fix this */;
            double  pTurning = turningSpeed * dt;
            headingAngle = headingAngle + pTurning; /* + TODO: fix this */;
