...
public:
    ...
    void solve(int nSteps = 10) {
        ...
        for (uint i = 0; i < nSteps; i++) {
            ...
            Matrix dpdq, dpdq_init, dpdq_estimated, dpdq_last;
            for(int j =0; j< endEffectorTargets.size(); j++){
                
                
                gcrr.compute_dpdq(endEffectorTargets[j].p, endEffectorTargets[j].rb, dpdq_init);
                dpdq_last = dpdq_init.block(0,6,3,q.size() - 6).eval();
                P3D eeCurrent = gcrr.getWorldCoordinates(endEffectorTargets[j].p, endEffectorTargets[j].rb);
                dpdq = dpdq_last;
                deltaq += (dpdq.transpose() * dpdq).ldlt().solve(dpdq.transpose() * V3D (endEffectorTargets[j].target - eeCurrent));
            }
